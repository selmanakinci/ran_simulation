...
class RanSimEnv1(gym.Env):
    ...
    def __init__(self, C_algo = 'RL', t_final: int = 10000):
        """
        Main ran_simulation
        """
        ...
        sim_param.update_seeds(new_seed)
    ...
    def step(self, action):
        ...
        # check done
        if (slices[0].sim_state.now % 100) == 0:
            done = 1
        else:
            done = 0
        done = bool(done)
    ...
    def reset(self, **kwargs):
        ...
        # get next state
        #  -------------------------------------------------------
        # method 0 tp2, non-zero initial state
